...
export default function Home() {
  ...
  async function libertarReservaExpirada() {
  ...
  useEffect(() => {
    ...
    // Não descer o temporizador se estiver a carregar o ficheiro
    if (etapa === 'pagamento' && tempoRestante > 0 && !carregando) {
      timer = setInterval(() => {
        setTempoRestante((prev) => prev - 1);
      }, 1000);
    } else if (etapa === 'pagamento' && tempoRestante <= 0) {
      toast.error('A sua reserva expirou.');
      libertarReservaExpirada();
    }

    return () => clearInterval(timer);
  }, [etapa, tempoRestante, carregando]);
  ...
  function resetarEstado(): void {
    ...
    setTempoRestante(TEMPO_RESERVA);
    setFicheiro(null);
    setCarregando(false);
    setMensagemStatus('');
  }
  ...
  async function submeterFormulario(e: FormEvent<HTMLFormElement>): Promise<void> {
    ...
    if (error) {
      toast.error('Erro ao reservar o número. Pode já ter sido selecionado por outra pessoa.');
      resetarEstado();
    } else {
      setTempoRestante(TEMPO_RESERVA);
      setEtapa('pagamento');
  ...
  async function submeterComprovativo(e: FormEvent<HTMLFormElement>): Promise<void> {
    ...
    try {
      ...
      if (uploadError) {
        toast.error('Erro ao enviar o ficheiro do comprovativo.');
        setCarregando(false);
        return;
      }
      ...
      const { error: dbError } = await supabase.from('rifas').update({ comprovativo_url: urlFinal }).eq('numero', numeroSelecionado);

      if (dbError) {
        toast.error('O ficheiro foi submetido, mas ocorreu um erro ao associar ao número.');
        console.error('Erro BD:', dbError);
      } else {
        setMensagemStatus('Comprovativo enviado com sucesso! A sua reserva está a aguardar validação.');
        // toast.info('Comprovativo associado com sucesso!');
      }
    } catch (err) {
      console.error(err);
